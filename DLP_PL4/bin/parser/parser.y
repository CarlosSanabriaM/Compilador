...
variable_definition: variables ':' type ';'							{	// Una definicion de variable puede tener varias variables, y tenemos que dividirlo en varias VarDefinition
																		...
																		List<Variable> vars = (List<Variable>) $1;
																		for(Variable var : vars){
																			VarDefinition varDef = new VarDefinition(scanner.getLine(), scanner.getColumn(), var.getName(), (Type) $3);
...
variables: ID														{	// TODO - MEJOR CREAR UNA LISTA DE STRINGS QUE UNA LISTA DE VARIABLES!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
																		Variable var = new Variable(scanner.getLine(), scanner.getColumn(), (String) $1);
																		$$ = asVariableList(var);
																	}
		| variables ',' ID											{	
...
struct_body: variable_definition 									{	List<RecordField> fields = new LinkedList<RecordField>(); 
		...
		| struct_body variable_definition							{	List<RecordField> fields = (List<RecordField>) $1; 
...
// * Para añadir una Variable a un List<Variable> de un solo elemento
private List<Variable> asVariableList(Variable variable){
